...
const blob = await R(`(async()=>{
  const T = {
    ...
    cobertura: Object.assign(structuredClone(NEXO_CATALOGO.PLANES.professional.base), {reparacionesIncluidas:[], incluirSLA:true}),
  ...
  m.etiquetaPub = "[Nombre del plan]";
  m.pie = "Esta propuesta se rige por los Términos y Condiciones Generales del Servicio aquí incluidos. Los campos [entre corchetes] se completan al perfeccionar el acuerdo.";
  return await armarDocx(window.docx, m, T);
})()`);
const buf = Buffer.from(await blob.arrayBuffer());
const out = "/Users/Maxi/Library/CloudStorage/OneDrive-Personal/2. Comercial/Marketing y Diseño/Nexolibre - Plantilla Contrato Mantenimiento MRI-CT.docx";
