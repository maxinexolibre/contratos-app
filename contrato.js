/* ============================================================
   Nexolibre · Modelo de contenido del contrato
   ------------------------------------------------------------
...
function construirContrato(c, opts = {}) {
  ...
  // ---------- Encabezado ----------
  const etiquetaPub = esc(c.plan?.etiquetaPublica) || CAT.PLANES[c.plan?.id]?.etiquetaPublica || "Programa de Mantenimiento MRI / CT";
  const kicker = `${esContrato ? "CONTRATO DE SERVICIO" : "PROPUESTA DE SERVICIO"}${V(c.numero, "N.º") ? " · " + V(c.numero, "N.º") : ""}`;
  ...
  const preambulo = `Entre <b>Nexolibre</b> («El Prestador») y <b>${T(rsp.razonSocial, "razón social")}</b>${cuitPre ? `, CUIT ${cuitPre}` : ""}${domic}${fantasia}${otras} («El Cliente»), se celebra el presente Contrato de Prestación de Servicios de Mantenimiento, que se regirá por las cláusulas siguientes:`;

  // ---------- Cláusulas ----------
  // `si: false` la deja fuera y renumera el resto automáticamente.
  const defs = [
    { id: "objeto", titulo: "OBJETO", cuerpo: `El Prestador se obliga a prestar los servicios de mantenimiento preventivo y correctivo sobre los equipos de diagnóstico por imágenes de propiedad del Cliente individualizados en la(s) Ficha(s) Técnica(s) (los «Equipos»), conforme al alcance${incluirSLA ? ", niveles de servicio" : ""} y condiciones aquí establecidos.` },
    ...
    { id: "alcance", titulo: "ALCANCE DEL SERVICIO", cuerpo: `El servicio incluye, para cada Equipo: ${alcance.map((a) => a.replace(/\.$/, "")).join("; ")}.${reparaciones.length ? ` Reparaciones de partes expresamente incluidas: ${reparaciones.map((r) => r.replace(/\.$/, "")).join("; ")}.` : ""}${cupoBobinas} La provisión de repuestos y partes ${cov.partesIncluidas ? "se encuentra incluida según Ficha Técnica" : "<b>no está incluida</b> y se rige por la {{repuestos}}"}.` },
    ...
    { id: "exclusiones", titulo: "EXCLUSIONES", cuerpo: `No se encuentran incluidos: ${exclusiones.map((a) => a.replace(/\.$/, "")).join("; ")}. Todo lo no detallado expresamente en este contrato y sus Fichas Técnicas.` },
  ...
  return {
    ...
    subtitulo: "Programa de mantenimiento preventivo y correctivo · MRI / CT",
    ...
    pie: esContrato ? "Copia del contrato suscripto." : "Esta propuesta se rige por los Términos y Condiciones Generales del Servicio aquí incluidos.",
  };
}
